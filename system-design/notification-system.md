...
> **Core problem:** Build an event-driven fanout system that takes user actions, fans them out to multiple delivery channels, and guarantees reliable delivery with retries, preferences, and prioritisation — without spamming users or overwhelming external providers.
...
**A notification system is an event-driven pipeline that takes user actions, fans them out to multiple delivery channels, and ensures reliable delivery with retries and preferences.**
...
Every notification must be fanned out from one event to potentially millions of recipients. Two models:

| | Push (fan-out on write) | Pull (fan-out on read) |
|---|---|---|
| When | Event fires → generate all notifications immediately | User opens app → fetch their notifications |
| Latency | Low read latency (pre-generated) | High read latency (generated on demand) |
| Write cost | High — 50M users × 1 write = 50M DB inserts | Low — one row written at event time |
| Best for | Small to medium scale; real-time feel | Large-scale bulk campaigns; celebrity senders |
| Breaks when | Bulk campaign = 50M writes = DB overwhelmed | App opened by 10M users at once = read storm |

**Reality:** This system uses **push for OTP/transactional** (one recipient, instant delivery) and **pull-heavy via Kafka fan-out for bulk promotional** (50M recipients, async batch). The Kafka topic is the fan-out mechanism — one event in, N consumer reads out.

### Two write paths, one critical distinction:
...
- **Idempotency:** `notification_id` is a UUID generated by the client and sent in the request. If the same `notification_id` is received twice (client retry after timeout), Notification Service detects the duplicate via `UNIQUE(notification_id)` and returns the existing record — no duplicate delivery.
...
>
> **This system uses at-least-once + idempotency key = effectively-once.**
> - `notification_id` (UUID) is unique per notification. Delivery Consumer uses it as an idempotent key when writing status to Notification DB (`INSERT ... ON CONFLICT notification_id DO NOTHING`). Duplicate Kafka events from consumer replay never cause duplicate DB writes or duplicate deliveries.
>
...
> - Retry: exponential backoff (1s → 2s → 4s) + DLQ after 3 failures
>
...
> - Trade-off accepted: at-most-once on Kafka crash. Acceptable — user can request OTP resend; resend is a product feature for exactly this scenario.
>
...
1. Client calls `POST /notifications/send` with `priority: standard`. I route this to the **Notification Service** via API Gateway — the gateway validates the API key, applies rate limiting, and routes by URL.
   ...
   **Rate limiting is mandatory here at two levels** (connecting to the distributed rate limiter pattern):
   - **Inbound (API Gateway):** Token bucket per `clientId` — prevents a single client (Flipkart) from flooding the platform and starving other clients (Uber). E.g., max 100K notifications/min per client.
   - **Outbound (Provider level):** Token bucket per external service — SendGrid = 100 emails/sec, Twilio = 1K SMS/sec. Without this, one bulk campaign exceeds provider rate limits and messages are silently dropped with no error surfaced to the client.
...
Real systems must support: opt-in/out per channel, do-not-disturb windows, and channel priority ordering.
...
| channel | enum | email / sms / push (one row per channel) |
| enabled | boolean | default true — user opt-in/out |
| priority | int | 1=first choice, 2=fallback, 3=last resort |
| do_not_disturb_start | time | e.g., 22:00 — no notifications after this |
| do_not_disturb_end | time | e.g., 08:00 — resume notifications after this |
| updated_at | timestamp | |

**Why one row per channel (not one row per user)?**
A user may enable email but disable SMS, and set push as priority 1. Storing per-channel rows makes `WHERE external_user_id = ? AND channel = 'sms' AND enabled = true` a simple indexed lookup. A single JSON blob per user requires deserialisation on every check.

**Redis cache key:** `pref:{clientId}:{userId}:{channel}` → `{enabled, priority, dnd_start, dnd_end}` (TTL=60s)
...
### Universal Retry Pattern: Retry → Fallback → Dead Letter Queue

Every provider failure follows the same three-stage recovery:
...
This pattern is why `priority` exists in the user preference table — channel 1 = first attempt, channel 2 = fallback. The system automatically escalates.

### Failure Table

| Failure | Impact | Recovery |
|---|---|---|
| **External service (Twilio) down** | SMS not delivered | Retry (1s, 2s, 4s backoff) → fallback to push notification → DLQ after 3 attempts → ops alert |
| **External service (SendGrid) delayed** | Email delayed, not lost | At-least-once via Kafka — consumer retries on timeout; idempotency key prevents duplicate delivery on retry |
| **Push (FCM) silently fails** | Push not delivered, no error | Webhook receipt never arrives → reconciliation job detects `status=SENT` with no delivery receipt after 24h → mark as FAILED |
| Notification Service crashes after DB write, before ACK | Client sees timeout | Client retries with same `notification_id`; UNIQUE constraint detects duplicate → return existing record, no re-send |
| Kafka crashes (brief) — OTP path | OTP in-flight event lost | User requests resend → new event enters pipeline |
| Kafka crashes (brief) — standard path | Event in outbox (published=false) | Debezium replays from WAL on restart; no message loss |
| CDC pipeline lag | Promo notifications delayed | Alert at P95 lag >5s; scale Debezium connectors |
| User Preference Redis down | Preferences cold-read from DB | Auto-failover to DB; performance degrades, correctness maintained |
| Delivery webhook fails | Status stays `SENT`, never `DELIVERED` | Reconciliation job polls external provider's delivery report API; updates status |
| BigQuery ingest failure | Event log incomplete | Kafka 7-day retention; re-stream on recovery |
