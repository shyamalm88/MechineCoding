...
```mermaid
graph TD
    Call["Provider calls external service"] -->|FAIL - timeout / 5xx| Stage1["Stage 1: Retry with exponential backoff - attempt 1, wait 1s, attempt 2, wait 2s, attempt 3, wait 4s"]
    Stage1 -->|still failing after 3 attempts| Stage2["Stage 2: Channel fallback, if configured - SMS failed -> try Push, Email failed -> try SMS"]
    Stage2 -->|no fallback channel, or fallback also failed| Stage3["Stage 3: Dead Letter Queue (DLQ Kafka topic) - ops team alerted, manual inspection or replay"]
```
...
> [!NOTE]
> Key Insight: Beyond raw throughput, async also isolates the client-facing SLA from provider health — a Twilio outage becomes a Kafka backlog to drain down once service resumes, not a wave of client-visible timeouts propagating back through the Notification Service.
...
> [!NOTE]
> Key Insight: Without single-transaction atomicity, a crash between the two writes could leave a notification row with no matching outbox row to publish it — a silently unrecoverable gap that Debezium's CDC has no way to detect or repair after the fact. That failure mode, not the throughput comparison, is what makes the transactional guarantee non-negotiable here.
...
```mermaid
graph TD
    subgraph "Fast Path (OTP / Critical)"
        F1["Client request"] --> F2["API Gateway (auth + rate limit)"]
        F2 --> F3["Notification Service"]
        F3 --> F4["Kafka critical-{channel} topic (direct, no outbox)"]
        F4 --> F5["OTP Provider (dedicated, more replicas, lower lag)"]
        F5 --> F6["Twilio / FCM"]
        F6 --> F7["User in under 5 seconds"]
    end
    subgraph "Reliable Path (Standard / Promotional)"
        R1["Client request"] --> R2["API Gateway"]
        R2 --> R3["Notification Service"]
        R3 --> R4["DB Transaction: notification(PENDING) + outbox(UNPUBLISHED) - COMMIT"]
        R4 --> R5["ACK to client - message is now durable"]
        R5 --> R6["CDC Pipeline detects outbox row"]
        R6 --> R7["Kafka standard/promo-{channel} topic"]
        R7 --> R8["Provider checks Redis preference cache"]
        R8 --> R9["External service (SendGrid / FCM)"]
        R9 --> R10["Webhook"] --> R11["Kafka delivery-status"]
        R11 --> R12["Delivery Consumer"] --> R13["PostgreSQL (status=DELIVERED) + BigQuery (event log)"]
    end
```
