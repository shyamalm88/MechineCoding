...
> [!NOTE]
> **Key Insight:** Checkout is the one system boundary where nothing is allowed to read from the config cache — every other read path in this design tolerates staleness somewhere, but the price a customer actually pays cannot.
