...
Converts any folder of `.md` files into a browsable static HTML site — see `tools/md-site/README.md` for full usage. Two sources currently use it: `Theory/*.md` (→ `theory-notes/`) and `system-design/*.md` (→ `system-design-notes/`). Both source folders are kept in the repo (not deleted after generating) so new notes can just be added as `.md` files and the site regenerated — hand-editing generated HTML directly is not worth it. The sidebar is not duplicated per page: every page ships an empty mount point plus a shared `assets/sidebar.js`, which renders the nav client-side from a single generated `assets/nav.json` — so adding a note or changing a category/star rating only changes that one JSON file, not every page. Regenerate after editing:
```bash
